...
```{r, comment=NA, fig.width = 10, fig.height=5 , message=FALSE}
...
```{r, comment=NA}
#str(market.survey) # not avaialbe - try "terr" version of S-PLUS from Tibco?
str(Wage)

fit <- gam(I(wage > 250) ~ s(age, k = 6), data = Wage, family = binomial)
summary(fit)

#with(Wage, plot(age, wage > 250, type = "n", ylim = c(0, 0.06)))
with(Wage, plot(age, wage > 250, type = "n", ylim = c(0, 0.05)))
#axis(2, at = c(0, 1))
#with(Wage, points(jitter(age), wage > 250, pch = "|", col = "grey"))
#with(Wage, points(jitter(age[wage > 250]), rep(0, length(age[wage > 250])), pch = "|", col = "grey"))
...
#gam(pick == "ATT" ~ s(usage), family = binomial)

#mkt.fit1 <- gam(pick == "ATT" ~ s(log(usage + 2)), family = binomial)

#plot(usage, picks == "ATT", type = "n", ylim = c(-0.1, 1.1), yaxt = "n")
#axis(2, at = c(0, 1))
#points(jitter(usage), pick == "ATT", pch = "|")
#o <- order(usage)
#lines(usage[o], fitted(mkt.fit1)[o])
...
#axis(2, at = c(0, 1))
#with(Wage, points(jitter(age), wage > 250, pch = "|", col = "grey"))
#with(Wage, points(jitter(log(age[wage > 250])), rep(0, length(age[wage > 250])), pch = "|", col = "grey"))
...
```{r, comment=NA, fig.width=5}
str(kyphosis)

# default df = 4, so k = 5 with fx = TRUE
kyph.gam1 <- gam(Kyphosis ~ s(Age, k = 5, fx = TRUE) + 
                            s(Number, k = 5, fx = TRUE) + 
                            s(Start, k = 5, fx = TRUE), family = binomial, data = kyphosis)
...
#plot(kyph.gam1, residuals = FALSE, rug = FALSE)
...
#plot(kyph.gam2, residuals = TRUE, rug = FALSE)
par(mfrow = c(1, 2))
...
```{r, comment=NA, fig.width=10, fig.height=5}
# fit2 <- gam(I(wage > 250) ~ s(year, k = 5) + s(age, k = 6) + education, data = Wage, family = binomial)
# summary(fit2)
# plot(fit2)
# 
# fit2 <- gam(I(wage > 250) ~ year + s(age, k = 6) + education, data = Wage, family = binomial, subset =( education !="1. < HS Grad"))
# summary(fit2)
# plot(fit2)

fit2 <- gam(I(wage > 250) ~ s(age, k = 6) + education, data = Wage, family = binomial, subset = (education != "1. < HS Grad"))
summary(fit2)

fit3 <- gam(I(wage > 250) ~ log(age) + education, data = Wage, family = binomial, subset = (education != "1. < HS Grad"))
summary(fit3)
...
#eth3 <- gam(NOx ~ s(C, E, k = 12), data = ethanol)
#summary(eth3)

eth3 <- gam(NOx ~ s(C, k = 3) + s(E, k = 13), data = ethanol)
summary(eth3)

#eth3 <- loess(NOx ~ C + E, span = 0.25, data = ethanol)
#summary(eth3)
...
wage.fit <- gam(I(wage > 250) ~ s(age, bs = "ts") + sex + maritl + race + education + region + jobclass + health + health_ins, data = Wage, gamma = 1.4)
summary(wage.fit)

# Same again using REML/ML
wage.fit2 <- gam(I(wage > 250) ~ year + s(age, bs = "ts") + maritl + race + education + jobclass + health + health_ins, data = Wage, method = "REML")
summary(wage.fit2)

# And once more, but using the null space penalization
wage.fit3 <- gam(I(wage > 250) ~ year + s(age, bs = "cr") + maritl + race + education + jobclass + health + health_ins, data = Wage, method = "REML", select = TRUE)
summary(wage.fit3)

# Try bs = "ad" - adaptive
# wage.fit4 <- gam(I(wage > 250) ~ year + s(age, bs = "ad") + maritl + race + education + jobclass + health + health_ins, data = Wage)
# summary(wage.fit4)
...
# Reduced model
# wage.fit5 <- gam(I(wage > 250) ~ s(age, bs = "ad") + Married + Degree + jobclass, data = Wage)
# summary(wage.fit5)
wage.fit4 <- gam(I(wage > 250) ~ s(age, bs = "ad") + Married + Degree + jobclass, data = Wage, method = "REML", select = TRUE)
summary(wage.fit4)
...
persp(new.eth$C, new.eth$E, eth.grid, theta = 315, phi = 20, ticktype = "detailed", expand = 0.375)

#library(rgl)
#surface3d(new.eth$C, new.eth$E, eth.grid)
...
#Z <- outer(C, E, FUN = function(x, y) predict(eth3, data.frame(x, y)))

#outer(X=C, Y=E, FUN=Vectorize(predict, vectorize.args=c("C", "E")), object=eth3, data=Z)
