...
Additive regression model: $\eta(X) = \alpha + f_1(x_1) + f_2(x_2) + ... + f_p(x_p)$

Examples:

- Simple Additive: $y = f(x) + \epsilon$
    
- Semiparametric: $y = X^t\beta + f(z) + \epsilon$
    
- Additive: $y = \beta^tX + f_1(z_1) + f_2(z_2) + ... + f_q(z_q) + \epsilon$

- Nonparametric logistic regression: $logit(P(X)) = log\left(\frac{P(X)}{1 - P(X)}\right) = \eta(X)$
...
```{r, comment=NA, fig.width = 11, fig.height=5.5, warning=FALSE}
str(car.test.frame)
...
```{r, comment=NA, fig.width=11, fig.height=5.5}
...
```{r, comment=NA, fig.width=5.5, fig.height=5.5}
str(Wage)
...
```{r, comment=NA, fig.width=11, fig.height=3.66}
str(kyphosis)
...
```{r, comment=NA, fig.width=8.5, fig.height=8.5}
summary(kyph.gam1)
...
```{r, comment=NA, fig.width=5.5, fig.height=5.5, warning=FALSE}
...
```{r, comment=NA, fig.width=8.5, fig.height=8.5, warning=FALSE}
...
```{r, comment=NA, fig.with=11, fig.height=5.5}
formula(eth3)
